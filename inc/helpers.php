...
function addACarStepParse($paramms)
{
	...
	if ($forApps['ca-year']) $forApps['year'] = ($forApps['ca-year']) ? $forApps['ca-year'] : [];
...
function stm_ma_add_a_car($update = false)
{
	...
	foreach ($stepOne as $k) {
		if (empty($k) || $k == 'add_media') continue;
		...
		if (empty($_POST['stm_f_s_' . $k])) $not_send[] = 'stm_f_s_' . $k;
	}
	...
	foreach ($_POST as $second_step_key => $second_step_value) {
		if (empty($second_step_key)) continue;
	...
	foreach ($_POST as $third_step_key => $third_step_value) {
		if (empty($third_step_key)) continue;
...
function stm_ma_add_a_car_media_two($user_id, $token, $post_id)
{
	...
	if (intval(get_post_meta($post_id, 'stm_car_user', true)) != intval($user_id)) {
		...
		wp_send_json(array('message' => esc_html__('You are trying to add car to another car user, or your session has expired, please sign in first user id ' . $user_id . ' post id ' . $post_id . ' user token: ' . $token . ' post author: ' . get_post_meta($post_id, 'stm_car_user', true), 'stm-motors-application')));
		exit;
	}
